...
function checkFormField(forma) {
  ...
  const validateTel = () => {
    ...
    else if (telInputValue.length < 4) elInput.nextElementSibling.innerHTML = "Поле, обязательное для заполнения";
  ...
  const validateText = () => {
    ...
    else if (name.value.length >= 30) name.nextElementSibling.innerHTML = "Число символов не должно превышать 30";
